...
int main(int argc, char *argv[])
{
...
#ifdef __PRINT_PROCESO
        printf("ADMIN --> La prioridad maxima está en otro nodo, tengo que enviar el testigo.\n");
#endif
    ...
    else
    {
...
#ifdef __PRINT_PROCESO
            printf("ADMIN --> la prioridad maxima está en mi nodo.\n");
#endif
            sem_post(&(me->sem_prioridad_maxima));
        ...
        { // misma prioridad mi nodo y otro nodo
#ifdef __PRINT_PROCESO
            printf("ADMIN --> Mi nodo y otro nodo tenemos la misma prioridad.\n");
#endif
            sem_post(&(me->sem_prioridad_maxima));
            sem_post(&(me->sem_prioridad_max_otro_nodo));
            sem_wait(&(me->sem_contador_procesos_max_SC));
            ...
            {
#ifdef __PRINT_PROCESO
                printf("ADMIN --> Quiero evitar la exclusión mutua o ya no procesos de esta prioridad en mi nodo.\n");
#endif
                sem_post(&(me->sem_contador_procesos_max_SC));
                if (me->contador_reservas_admin_pendientes > 0)
                {
